...
<body>
        ...
        <tbody>
            @foreach($factures as $facture)
                @php
                    ...
                    // On prend comme période de retard : date dépôt + délai légal → aujourd’hui ou date règlement
                    $dateDebut = $facture->date_depot
                        ? $facture->date_depot->copy()->addDays($facture->delai_legal_jours ?? 30)
                        : null;

                    $dateFin = $facture->date_reglement ?: Carbon\Carbon::now();

                    $result = $dateDebut
                        ? App\Services\InteretService::calculerInteretsPourPeriode($facture, $dateDebut, $dateFin)
                        : ['jours_retard' => 0, 'interet_ht' => 0, 'interet_ttc' => 0];
                @endphp
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @if($result['jours_retard'] > 0)
                            <span class="badge badge-warning">{{ $result['jours_retard'] }} jours</span>
                        @else
    ...
    <div style="margin-top: 20px; text-align: center; font-size: 10px; color: #666;">
        ...
        Total intérêts HT :
        {{ number_format(
    $factures->sum(function ($f) {
        if ($f->date_depot && is_null($f->date_reglement)) {
            $dateDebut = \Carbon\Carbon::parse($f->date_depot);
            $dateFin = now();

            return InteretService::calculerInteretsPourPeriode($f, $dateDebut, $dateFin)['interet_ht'];
        }
        return 0;
    }),
    2
